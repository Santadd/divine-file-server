...
export class BusinessFilesController {
  ...
  async getFiles(
    ...
    const builder = appDataSource
      .getRepository(BusinessFile)
      .createQueryBuilder()
      .orderBy("id", "DESC");

    const { records: businessFiles, paginationInfo } = await Paginator.paginate(
      builder,
      req
    );
  ...
  async getFile(
    ...
    const { id } = req.params;
    const businessFile = await appDataSource
      .getRepository(BusinessFile)
      .findOneByOrFail({
        id: id,
      });
  ...
  async downloadFile(req: Request, res: Response, next: NextFunction) {
    ...
    const mailRes = await MailService.sendMail({
      ...
      attachments: [
        {
          filename: `${businessFile.file}`,
          path: `./uploads/businessfiles/${businessFile.file}`,
        },
      ],
    });

    // If mail was delivered successfully
    if (mailRes) {
      // Start a transaction
    ...
    return ResponseUtil.sendResponse(
      res,
      "File has been sent successfully via email",
      null
    );
  }
  ...
  async downloadDetails(req: Request, res: Response, next: NextFunction) {
    ...
    const { records: fileDetails, paginationInfo } = await Paginator.paginate(
      queryBuilder,
      req
    );
    ...
    return ResponseUtil.sendResponse(
      res,
      "Download data fetched successfully",
      fileDetails,
      paginationInfo
    );
  }
  ...
  async allDetails(req: Request, res: Response, next: NextFunction) {
    ...
    const queryBuilder = appDataSource
      .getRepository(BusinessFile)
      .createQueryBuilder("businessfile");

    // Check if file exists
    await queryBuilder
      .where("businessfile.id = :bfileId", { bfileId: id })
      .getOneOrFail();
    ...
    queryBuilder
      ...
      .select(["businessfile", "download", "email"]);
